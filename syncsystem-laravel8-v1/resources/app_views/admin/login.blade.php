...
@section('cphBody')
    ...
    <div style="position: absolute;
        ...
        top: 50%;
        margin-top: -135px;
        background-image: url(/{{ config('app.gSystemConfig.configDirectoryFilesLayoutSD') }}/backend-layout-login-bg01.jpg), url(/{{ config('app.gSystemConfig.configDirectoryFilesLayoutSD') }}/backend-layout-login-bg02.jpg);
        background-position: top, bottom;
        background-repeat: repeat-x, repeat-x;"
    >
        {{-- Logo. --}}
        <img src="/{{ config('app.gSystemConfig.configDirectoryFilesLayoutSD') }}/backend-layout-logo-client.png"
            alt="Logo - {{ \SyncSystemNS\FunctionsGeneric::contentMaskRead(config('app.gSystemConfig.configSystemClientName'), 'config-application') }}"
        ...
        <main class="ss-backend-text01" style="position: absolute; display: block; right: 10px; bottom: 30px; text-align: right;">
            ...
            <form id="formLogin" name="formLogin" method="POST" action="/{{ config('app.gSystemConfig.configRouteBackend') . '/' . config('app.gSystemConfig.configRouteBackendLogin') }}" enctype="multipart/form-data">
                ...
                <div style="position: relative; display: block;">
                    {{ \SyncSystemNS\FunctionsGeneric::appLabelsGet(config('app.gSystemConfig.configLanguageBackend')->appLabels, 'backendLoginUser') }}:
                ...
                <div style="position: relative; display: block; margin: 5px 0px;">
                    {{ \SyncSystemNS\FunctionsGeneric::appLabelsGet(config('app.gSystemConfig.configLanguageBackend')->appLabels, 'backendLoginPassword') }}:
                ...
                <div style="position: relative; display: block;">
                    <button class="ss-backend-btn-base ss-backend-btn-action">
                        {{ \SyncSystemNS\FunctionsGeneric::appLabelsGet(config('app.gSystemConfig.configLanguageBackend')->appLabels, 'backendButtonLogin') }}
                    </button>
                </div>
            </form>
        </main>
        ...
        >
            {{
                \SyncSystemNS\FunctionsGeneric::appLabelsGet(config('app.gSystemConfig.configLanguageBackend')->appLabels, 'layoutCopyright') . ' ® ' .
                \SyncSystemNS\FunctionsGeneric::appLabelsGet(config('app.gSystemConfig.configLanguageBackend')->appLabels, 'layoutDevName') . ' ' .
                \SyncSystemNS\FunctionsGeneric::appLabelsGet(config('app.gSystemConfig.configLanguageBackend')->appLabels, 'layoutCopyright1')
            }}
        </footer>
    </div>
@endsection
